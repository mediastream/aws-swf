...
EventList.prototype = {
  ...
  _has_event_with_attribute_value: function(attributeKey, attributeValue, eventType) {
   ...
   /**
    * Search for an event with the corresponding type that matches the scheduled activityId
    * and has a scheduledEventId >= minScheduledEventId
    * @param {String} eventType
    * @param {String} activityId
    * @param {Integer} minScheduledEventId - Minimum scheduledEventId to consider
    * @returns {Boolean}
    */
   _has_event_for_scheduledEventId_since: function(eventType, activityId, minScheduledEventId) {
      var attrsKey = this._event_attributes_key(eventType);
      return this._events.some(function (evt) {
          if (evt.eventType === eventType) {
             var scheduledEventId = parseInt(evt[attrsKey].scheduledEventId, 10);
             if (scheduledEventId >= minScheduledEventId &&
                 this.activityIdFor(scheduledEventId) === activityId) {
                return true;
             }
          }
       }, this);
   ...
   is_activity_started: function (activityId) {
      var lastScheduledEventId = this.last_scheduled_event_id_for_activity(activityId);
      if (lastScheduledEventId === -1) {
         return false;
      }
      return this._has_event_for_scheduledEventId_since("ActivityTaskStarted", activityId, lastScheduledEventId);
   },
   ...
   has_activity_timedout: function (activityId) {
      ...
      return this._has_event_for_scheduledEventId_since('ActivityTaskTimedOut', activityId, lastScheduledEventId);
   },
   ...
   has_activity_failed: function (activityId) {
      ...
      return this._has_event_for_scheduledEventId_since('ActivityTaskFailed', activityId, lastScheduledEventId);
   },
   ...
   has_schedule_activity_task_failed: function (activityId) {
      ...
      return this._has_event_for_scheduledEventId_since('ScheduleActivityTaskFailed', activityId, lastScheduledEventId);
   },
   ...
   is_activity_canceled: function (activityId) {
      ...
      return this._has_event_for_scheduledEventId_since("ActivityTaskCanceled", activityId, lastScheduledEventId);
   },
   ...
   /**
    * Return the eventId of the most recent ActivityTaskScheduled event for the given activityId
    * @param {String} activityId
    * @returns {Integer|false} - The eventId of the last scheduled event, or false if not found
    */
   last_scheduled_event_id_for_activity: function (activityId) {
      var i;
      var lastEventId = -1;
      for (i = 0; i < this._events.length; i++) {
         var evt = this._events[i];
         if (evt.eventType === "ActivityTaskScheduled" &&
             evt.activityTaskScheduledEventAttributes.activityId === activityId &&
             parseInt(evt.eventId, 10) > lastEventId) {
            lastEventId = parseInt(evt.eventId, 10);
         }
      }
      return lastEventId;
   },
   ...
   has_activity_completed: function (activityId) {
      ...
      return this._has_event_for_scheduledEventId_since('ActivityTaskCompleted', activityId, lastScheduledEventId);
   },
